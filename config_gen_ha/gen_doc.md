...
Скрипт генерирует набор YAML-файлов для Home Assistant из таблицы сигналов (DI/DO) и их MQTT-путей.

## Как запускать

- **Вход**: файл сигналов — один из форматов:
  - `.csv` (`;`-разделитель, кодировка UTF‑8) — работает и без pandas;
  - `.xlsx`, `.xls`;
  - `.ods`.
  Для `.xlsx`/`.xls`/`.ods` нужны pandas и движки (`openpyxl`, `xlrd`, `odfpy`;
  перечислены в `requirements.txt`). Берётся **первый лист**.
...
# или: --csv="signals.xlsx" / --csv="signals.ods"
```
